...
16. **句长方差（C16）**：优先贴近 `style-profile.json.sentence_length_std_dev`；若缺失则按 8-18 的人类常见波动控制。若出现 3 句及以上连续句长都落在 ±5 字内，必须主动打散至少 1 句
...
19. **对话意图约束（C19）**：每句对话都要能落到一个主要意图（试探 / 回避 / 施压 / 诱导 / 挑衅 / 敷衍等）；禁止书面腔对话、禁止用对白重复刚刚叙述过的信息，并用“去掉标签后仍能大致分辨说话人”做自测
