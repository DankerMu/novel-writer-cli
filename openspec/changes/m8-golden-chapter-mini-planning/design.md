...
现有项目 checkpoint `quickstart_phase="style"` 进入 F0（新行为，无需手动迁移）。已通过 F 的项目不受影响（checkpoint 在 F 之后，不会回退到 F0）。无需修改已有 vol-01 目录结构——如果 vol-01 已存在但无 L3 契约，F0 正常生成；如果已有契约（不太可能），则按现有 `quickstart_phase` / vol-01 产物恢复。
