...
- [x] 1.1 Audit all fixed quotas in `style-guide.md` Layer 2: identify every instance of "至少.*处", "每章.*次", "每.*次.*出现.*次", and similar fixed-count mandates
- [x] 1.2 Replace §2.2 角色语癖 frequency from "每 2-3 次对话出现 1 次" to "recurring but irregular, naturally varying frequency"
- [x] 1.3 Replace §2.3 反直觉细节 from "每章至少 1 处" to "naturally varying, presence encouraged but count not mandated"
- [x] 1.4 Review §2.5 句式多样性 and §2.6 标点符号约束 for any implicit fixed quotas; adjust language to emphasize rate ranges with natural variance acknowledgment
- [x] 1.5 Verify zero matches for quota patterns (`至少.*处`, `每章.*次`, `每.*次.*出现.*次`) in the updated file
...
- [x] 3.1 Add new §2.9 section header "人性化技法工具箱" after §2.8
- [x] 3.2 Document cognitive techniques: `thought_interrupt` (思维中断) and `self_correction` (自我纠正) with Chinese descriptions and examples
- [x] 3.3 Document sensory techniques: `sensory_intrusion` (感官侵入) and `mundane_detail` (琐碎细节) with Chinese descriptions and examples
- [x] 3.4 Document linguistic techniques: `dialect_slip` (方言滑落), `incomplete_sentence` (不完整句), `rhetorical_question` (反问) with Chinese descriptions and examples
- [x] 3.5 Document emotional techniques: `stream_of_consciousness` (意识流片段), `emotional_non_sequitur` (情感跳跃), `contradiction` (矛盾) with Chinese descriptions and examples
- [x] 3.6 Document structural techniques: `nested_parenthetical` (嵌套补充) and `abrupt_topic_shift` (突然转题) with Chinese descriptions and examples
- [x] 3.7 Add explicit random sampling instruction: "每章从工具箱中随机采样若干技法使用，不固定数量，不固定组合"
- [x] 3.8 Add categorization summary table: technique / category / Chinese name

## 4. Structural Rules (§2.10) — from anti-ai-polish.md

- [x] 4.1 Add new §2.10 section header "六层结构规则"
- [x] 4.2 L1 反模板句式: document forbidden patterns (三段式总结 / 二元对立 / 递进堆砌 / 排比开头) and alternatives (场景推进/对话推进/动作推进)
...
- [x] 7.1 Replace the existing 4-indicator 5-point scoring table with a 7-indicator 3-zone assessment table
- [x] 7.2 Define green/yellow/red zones for `blacklist_hit_rate` (per 1000 chars)
- [x] 7.3 Define green/yellow/red zones for `sentence_repetition_rate`
- [x] 7.4 Define green/yellow/red zones for `sentence_length_std_dev` (new, reference §2.8 and style-profile)
- [x] 7.5 Define green/yellow/red zones for `paragraph_length_cv` (new, reference §2.8 and style-profile)
- [x] 7.6 Define green/yellow/red zones for `vocabulary_diversity_score` (new)
- [x] 7.7 Define green/yellow/red zones for `narration_connector_count` (new, green = 0)
- [x] 7.8 Define green/yellow/red zones for `humanize_technique_variety` (new)
- [x] 7.9 Preserve old 4-indicator 5-point table as legacy fallback, clearly marked with backward compatibility note
...
- [x] 8.6 Verify §2.12 documents all 4 steps of polish execution flow
- [x] 8.7 Verify §2.13 quick checklist has exactly 5 items
- [x] 8.8 Verify Layer 4 table has exactly 7 indicators, each with 3 zones
- [x] 8.9 Verify backward compatibility: old 4-indicator table preserved as legacy
- [x] 8.10 Cross-reference with `docs/anti-ai-polish.md` — verify all content from sections 一~四 is represented
