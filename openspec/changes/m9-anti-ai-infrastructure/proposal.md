...
- **风格提取**（WorldBuilder Mode 7）不提取统计字段，style-profile 的统计维度永远是 null
...
2. **context-contracts**：CW manifest 新增 `inline.statistical_targets`（6 维统计目标，从 style-profile 提取）；QJ manifest 新增 `inline.statistical_profile`（CW 自报或 lint 产出的统计结果）；CW manifest 新增 `inline.genre_overrides`（从 platform-profile 或 concept.md 提取的类型覆写参数）。
...
4. **lint-structural.sh（新增）**：确定性结构规则 lint 脚本（from `anti-ai-polish.md` §2.10），检测：
   ...
   - 支持类型覆写参数输入（从 platform-profile.json genre 字段读取阈值）

5. **periodic-maintenance**：新增 max_words=250 上限执行规则（超出需人工审批）；新增人性化技法跨章追踪（同一技法连续 3 章以上触发告警）。

6. **world-builder Mode 7 step 2.5**：从样本文本提取 5 个统计字段填充 style-profile（sentence_length_std_dev、paragraph_length_cv、emotional_volatility、register_mixing、vocabulary_richness）。
...
- `world-builder-style-extraction`: Mode 7 step 2.5 提取统计字段
- `eval-schema`: 新增可选 anti_ai_statistical_profile + structural_rule_violations

## Impact

- 修改 6 个文件（quality-rubric.md、context-contracts.md、lint-blacklist.sh、periodic-maintenance.md、world-builder.md、labeled-chapter.schema.json）
