...
type VolumeStep = { pipeline: 'volume'; phase: 'outline' | 'validate' | 'commit' };
type QuickStartStep = { pipeline: 'quickstart'; phase: 'world' | 'characters' | 'style' | 'trial' | 'results' };
type ReviewStep = { pipeline: 'review'; phase: 'collect' | 'audit' | 'report' | 'cleanup' | 'transition' };
```
...
对缺少 `orchestrator_state` 的旧 checkpoint：
- 有 `inflight_chapter` → `WRITING`
- `pipeline_stage === 'idle'` 且 `current_chapter === 0` → `INIT`
- `pipeline_stage === 'idle'` 且 `current_chapter > 0` → `VOL_PLANNING`（保守推断）
- 其他 → `ERROR_RETRY`
...
function computeNextStep(checkpoint: Checkpoint): Step {
  const state = checkpoint.orchestrator_state ?? inferLegacyState(checkpoint);
  switch (state) {
    case 'INIT': return { pipeline: 'quickstart', phase: 'world' };
    case 'QUICK_START': return computeQuickStartNext(checkpoint);  // CS-O3
    case 'VOL_PLANNING': return computeVolumePlanNext(checkpoint); // CS-O2
    case 'WRITING': return computeChapterNext(checkpoint);         // existing
    case 'CHAPTER_REWRITE': return computeChapterNext(checkpoint); // existing
    case 'VOL_REVIEW': return computeReviewNext(checkpoint);       // CS-O4
    case 'ERROR_RETRY': return computeErrorRetryNext(checkpoint);
  }
}
```
