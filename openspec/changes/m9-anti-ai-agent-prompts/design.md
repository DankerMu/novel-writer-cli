...
4. 保持 Agent prompt 的简洁性——引用 style-guide section 编号而非内联完整规则内容
...
3) **QualityJudge `statistical_profile` 三个字段由 Agent 基于正文估算，当前 changeset 不新增额外统计输入契约**
   - issue 138 的范围是 prompt 升级而不是 context-contracts 扩展；因此 QJ 仅复用现有 `blacklist_lint`，其余统计值直接从正文估算。若后续要接入确定性统计输入，应在 follow-up changeset 中显式扩展 instruction packet 契约。
...
6) **StyleRefiner 类型覆写来源优先级：`project_brief` 显式“类型覆写” > `project_brief` 题材字段 > 默认值**
   - `platform_profile` 只提供平台节奏 / 驱动类型的辅助信号，不覆盖 brief 中的显式写法。这样与 CS-A2 的方法论单一事实来源保持一致。

7) **`narration_connector_count` 按 Layer 4 的 green / yellow / red 阈值执行，不做“>0 即 red”的超规格收紧**
   - 单个孤立命中仍是问题，但应标成 yellow；只有 2 个以上或连续多段靠连接词推进时才是 red。这样与 style-guide 的分区定义完全对齐。
...
- [Low] QJ 统计估算不准确（sentence_length_std_dev 等依赖 Agent 对文本的理解）→ Mitigation: 7 指标本身有 yellow 缓冲区；当前先用于 prompt-level 评审与趋势观察，后续如需确定性输入再走单独 changeset 扩展契约。
