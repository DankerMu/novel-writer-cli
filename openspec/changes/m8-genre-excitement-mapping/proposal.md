...
- `continue-skill-manifest`: Step 2.6 QualityJudge manifest：chapter≤3 注入 `inline.genre_golden_standards`（按 brief.genre 匹配）
- `start-skill-genre-selection`: Step A 题材选项新增"言情"为第 6 选项；选择后检查 genre×platform invalid_combinations 并 WARNING

## Impact

- 新增 2 个模板文件，修改 5 个文件（2 agents + 2 skills + 1 src），无新增 Agent：
...
| `skills/continue/SKILL.md` | Step 2.6: chapter≤3 injects `inline.genre_golden_standards` |
| `skills/start/SKILL.md` | Step A adds 言情; post-selection genre×platform check + WARNING |
