...
- 写法提醒：让单句段、短段、中段交错出现，不要每段都像同一个模板裁出来
...
- 回退目标：叙述段落为 0；若必须保留，也应是孤立个例而不是推进骨架
...
|------|-------------------------|----------|---------|
| `sentence_length_variance` | `sentence_length_std_dev` | 8-18 | `< 5`，句长过匀 |
...
| `register_mixing` | `register_mixing` | `medium` | `low`，整章播音腔 |
...
- `thought_interrupt` / 思维中断：角色的思路被外界刺激突然打断，留下不完整心流。例：`他刚想到“也许还能谈”——窗外那声急刹把后半句硬生生掐断。`
...
- 覆写来源优先读取 `concept.md` 中的“类型覆写”小节；若未提供，再读取 `platform-profile.json` 的 `genre` 字段
- 只要命中类型覆写，类型参数优先于默认值

| 类型 | L5 段落结构覆写 | L6 标点节奏覆写 | 额外说明 |
|------|-----------------|-----------------|----------|
| 科幻 | 单句段 15%-30%；单段可放宽到 120 字 | 感叹号 ≤5/章 | `难以形容` / `不可名状` 灰度允许，建议 ≤2/章且尽量补具体感官 |
| 悬疑 | 单句段 20%-35%；段长上限维持 100 字但更强调断点 | 使用默认值 | 优先保留疑点与停顿，不要把信息一次说满 |
...
对照 §2.1 的 10 类黑名单逐项扫：总结概括词、枚举模板词、学术腔、叙述连接词、情绪直述、动作套话、环境套话、叙事填充、抽象空词、机械开合词。
...
| `sentence_length_std_dev` | 8-18，或落在 style-profile 目标附近 | 5-8 或 18-24 | <5（过匀） |
| `paragraph_length_cv` | 0.4-1.2，或落在 style-profile 目标附近 | 0.3-0.4 或 1.2-1.5 | <0.3（过匀） |
| `vocabulary_diversity_score` | 数值化时 ≥0.45；若仅有枚举，则 `vocabulary_richness=high` | 数值化时 0.35-0.45；或枚举 `medium` | 数值化时 <0.35；或枚举 `low` |
| `narration_connector_count` | 0 | 1-2 个孤立命中 | ≥3 个，或连续多段靠连接词推进 |
| `humanize_technique_variety` | 单章出现 ≥3 种不同技法 | 单章只有 1-2 种技法 | 0，几乎没有人性化痕迹 |
