...
当 F0 的规划种子通过校验并提交后，Quick Start 会进入 `quickstart:trial`。这一步会按 `volumes/vol-01/` 里已经落地的 L3 契约、卷大纲和伏笔计划，连续试写第 1–3 章。
...
随后 `quickstart:results` 会汇总三章的质量结果，给出“进入正式卷规划 / 调整风格 / 重新试写”等决策入口。也就是说，F0 负责把前三章“先规划清楚”，Step F 负责把这三章“按规划写出来并评掉”。
