...
| Skill | 负责状态 | 说明 |
|-------|---------|------|
| `/novel:start` | INIT → QUICK_START, VOL_PLANNING, VOL_REVIEW | 入口适配层（thin adapter）：循环调用 `novel next/instructions` 获取 step + instruction packet，并派发对应 agent；状态机/确定性编排由 CLI 提供 |
| `/novel:continue` | WRITING（含内嵌门控 + 修订循环） | 续写适配层（thin adapter）：同样通过 `novel next/instructions` 驱动章节流水线与门控/修订循环；skill 不实现状态判断/路由 |
